...
def get_nonce(url):
    ...
    return re.search(
        rb'<input type="hidden" name="nonce" value="(.*?)">', res.content
    ).group(1)
...
def iter_challenges(url):
    ...
    if res.ok:
        ...
        for challenge in challenges:
            ...
            challenge_json = session.get(
                urljoin(url, "/api/v1/challenges/%d" % challenge["id"])
            ).json()
...
def run(url, username, password):
    ...
    for challenge in iter_challenges(url):
        ...
        challenge_path = path.join(category_path, challenge["name"].strip())
        ...
        if not path.exists(challenge_path):
            ...
            with codecs.open(
                ...
                logger.info(
                    "Creating Challenge [%s] %s"
                    % (challenge["category"] or "No Category", challenge["name"])
                )
            ...
            for file_name in challenge["files"]:
                fn = path.basename(file_name)
                download_file(
                    urljoin(url, "/files/%s" % file_name), path.join(challenge_path, fn)
                )
...
def main(args=None):
    ...
    # Proxy Configuration
    parser.add_argument("url", help="CTFd URL (For example: https://demo.ctfd.io/)")
    parser.add_argument("-u", "--username", help="CTF username", required=True)
    parser.add_argument("-p", "--password", help="CTF password.", required=True)
